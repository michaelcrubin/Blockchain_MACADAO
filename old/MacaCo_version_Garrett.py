...
def mine_block():
    ...
    MacaCo.add_transaction(sender = node_address, receiver = 'GARRETT', amount = 1)
...
def is_valid():
    ...
    if is_valid:
        response = {'message': 'The MacaCo Chain is valid.'}
    else:
        response = {'message': 'The MacaCo Chain is not valid. some inconsistencies found.'}
    return jsonify(response), 200
...
def add_transaction():
    json = request.get_json() # This guy gets a json file names nodes and assings it to json
...
def connect_node():
    json = request.get_json() # This guy gets a json file names nodes and assings it to json later to nodes
    nodes = json.get('nodes')
    if nodes is None:
        return "No new Node", 400
    for node in nodes:
        MacaCo.add_node(node)
    response = {'message': 'All new nodes are connected. The MacaCo Chain contains the following nodes:',
                'all_nodes': list(MacaCo.nodes)}
    return response
   # return jsonify(response), 201
